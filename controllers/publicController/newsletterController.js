...
export const subscribe = async (req, res) => {
  ...
  try {
    ...
    if (!doc) {
      ...
      const unsubscribeLink = `${process.env.WEBSITE_BASE_URL || "https://tmgdubai.ae"}/unsubscribe?token=${encodeURIComponent(token)}`;
const { html, text } = buildSubscriptionEmail({ email, unsubscribeLink });
    ...
    if (doc.status === "unsubscribed") {
      ...
      const unsubscribeLink = `${process.env.WEBSITE_BASE_URL || "https://tmgdubai.ae"}/unsubscribe?token=${encodeURIComponent(newToken)}`;
const { html, text } = buildSubscriptionEmail({ email, unsubscribeLink });
...
export const unsubscribe = async (req, res) => {
  try {
    ...
    try {
      const unsubscribeLink = `${process.env.WEBSITE_BASE_URL || "https://tmgdubai.ae"}/unsubscribe?token=${encodeURIComponent(doc.unsubscribeToken || token)}`;
